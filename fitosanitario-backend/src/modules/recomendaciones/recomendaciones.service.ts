...
export class RecomendacionesService {
  ...
  async update(id: number, dto: Partial<CreateRecomendacionDto>, usuarioId: number, userRol: string) {
    const rec = await this.repo.findById(id);
    ...
    if (dueñoId !== usuarioId && userRol !== 'MODERADOR') {
      throw new ForbiddenException('No tienes permiso para editar esta recomendación');
    }
  ...
  async remove(id: number, usuarioId: number, userRol: string) {
    ...
    const dueñoId = rec.usuario?.id;
    if (dueñoId !== usuarioId && userRol !== 'MODERADOR') {
      throw new ForbiddenException('No tienes permiso para eliminar esta recomendación');
    }
  ...
  async valorar(recomId: number, dto: CreateValoracionDto, usuarioId: number) {
    ...
    if (!rec) throw new NotFoundException(`Recomendación #${recomId} no encontrada`);

    const existente = await this.repo.findValoracionByUsuario(recomId, usuarioId);
    if (existente) {
      throw new BadRequestException('Ya has valorado esta recomendación');
    }

    await this.repo.crearValoracion(recomId, usuarioId, dto.puntuacion, dto.comentario);
