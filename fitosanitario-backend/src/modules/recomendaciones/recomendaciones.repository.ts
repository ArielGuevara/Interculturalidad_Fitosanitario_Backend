...
export class RecomendacionesRepository {
  ...
  async findAll(filtros?: { tipo?: string; cultivoId?: number; plagaId?: number }) {
    const conditions = [eq(schema.recomendacionesComunidad.activo, true)];

    if (filtros?.tipo) {
      conditions.push(eq(schema.recomendacionesComunidad.tipo, filtros.tipo as any));
    }
    ...
    return this.db
      ...
      .leftJoin(schema.usuarios,             eq(schema.recomendacionesComunidad.usuarioId, schema.usuarios.id))
  ...
  async update(id: number, data: { titulo?: string; descripcion?: string; tipo?: 'RECOMENDACION' | 'CONSULTA' | 'CONOCIMIENTO_ANCESTRAL' }) {
    const result = await this.db
  ...
  async crearValoracion(recomendacionId: number, usuarioId: number, puntuacion: number, comentario?: string) {
    const result = await this.db
      .insert(schema.valoraciones)
      .values({ recomendacionId, usuarioId, puntuacion, comentario: comentario ?? null })
      .returning();
    return result[0];
  }
  ...
  async getValoraciones(recomendacionId: number) {
    return this.db
      ...
      .from(schema.valoraciones)
      .leftJoin(schema.usuarios, eq(schema.valoraciones.usuarioId, schema.usuarios.id))
  ...
  async actualizarPromedio(recomendacionId: number) {
    ...
    return { valoracionPromedio: Math.round(promedio * 100) / 100, totalValoraciones: total };
  }
}
