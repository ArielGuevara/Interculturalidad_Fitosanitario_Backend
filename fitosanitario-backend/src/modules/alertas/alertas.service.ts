...
export class AlertasService {
  ...
  // ── Zonas ──
  async findAllZonas() { return this.repo.findAllZonas(); }
  ...
  async createZona(data: any) { return this.repo.createZona(data); }
  ...
  // ── Parámetros ──
  async findAllParametros() { return this.repo.findAllParametros(); }
  ...
  async createParametro(data: any) { return this.repo.createParametro(data); }
  ...
  async create(dto: CreateAlertaDto) { return this.repo.create(dto); }
  ...
  async ejecutarDeteccion() {
    ...
    for (const brote of brotes) {
      ...
      const nivel = reportes.length >= 10 ? 'CRITICO' : reportes.length >= 5 ? 'ALTO' : 'MEDIO';
